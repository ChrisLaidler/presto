...
int main(int argc, char *argv[])
{
  /* Basic variables */
  /* long numpad; */
  double tb = 0.0, T, phase, amp, signal = 0.0, orbmaxt = 0.0;
  infodata idata;
  makedata mdata;
  ...
  printf("Calculating noise points...\n\n");
  ...
  setall(mdata.noiseRand1, mdata.noiseRand2);
  ...
  chkfileseek(datfile, 0, 0, 0);

  printf("\n\n");
  printf("Calculating signal points...\n\n");
  printf("Amount Complete = %3d%%",0);
  fflush(stdout);

  // Seed the random generator
  setall(mdata.signalRand1, mdata.signalRand2);

  // Reset percent marker
  oldper = 0;
