      ...
      for (int plain = 0; plain < cStack->noInStack; plain++)         // Loop through plains in stack
      {
        ...
        for (int step = 0; step < batch->noSteps; step++)             // Loop through Steps
        {
          ...
          if ( batch->flag & FLAG_MUL_TEX )
            mult42<<<dimGrid, dimBlock, 0, cStack->multStream>>>(d_plainData, cHInfo->width, cStack->strideCmplx, cHInfo->height, d_iData, cPlain->kernel->kerDatTex);
          else
            mult41<<<dimGrid, dimBlock, 0, cStack->multStream>>>(d_plainData, cHInfo->width, cStack->strideCmplx, cHInfo->height, d_iData, cPlain->kernel->d_kerData);
