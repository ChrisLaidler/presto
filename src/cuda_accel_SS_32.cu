...
__host__ void add_and_searchCU32(cudaStream_t stream, cuFFdotBatch* batch )
{
  ...
  if        ( FLAGS & FLAG_CUFFT_CB_OUT )
  {
    if      ( FLAGS & FLAG_ITLV_ROW )
      add_and_searchCU32_p<FLAG_CUFFT_CB_OUT | FLAG_ITLV_ROW> (dimGrid, dimBlock, stream, batch);
    else if ( FLAGS & FLAG_ITLV_PLN )
      add_and_searchCU32_p<FLAG_CUFFT_CB_OUT | FLAG_ITLV_PLN>  (dimGrid, dimBlock, stream, batch);
    else
    {
