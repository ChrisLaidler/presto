...
__host__  void convolveffdot10_p(dim3 dimGrid, dim3 dimBlock, int i1, cudaStream_t cnvlStream, cuFFdotBatch* batch, uint stack)
{
  ...
  switch (cStack->noInStack)
  {
    case 1  :
    {
      ...
      convolveffdot10_f<FLAGS,noSteps,1><<<dimGrid, dimBlock, i1, cnvlStream>>>(cStack->d_kerData , cStack->d_iData, cStack->d_plainData, cStack->width, cStack->inpStride, offset);
      break;
    }
    case 2  :
    {
      cudaFuncSetCacheConfig(convolveffdot10_f<FLAGS,noSteps,2>, cudaFuncCachePreferL1);
      convolveffdot10_f<FLAGS,noSteps,2><<<dimGrid, dimBlock, i1, cnvlStream>>>(cStack->d_kerData , cStack->d_iData, cStack->d_plainData, cStack->width, cStack->inpStride, offset);
      break;
    }
    case 3  :
    {
      cudaFuncSetCacheConfig(convolveffdot10_f<FLAGS,noSteps,3>, cudaFuncCachePreferL1);
      convolveffdot10_f<FLAGS,noSteps,3><<<dimGrid, dimBlock, i1, cnvlStream>>>(cStack->d_kerData , cStack->d_iData, cStack->d_plainData, cStack->width, cStack->inpStride, offset);
      break;
    }
    case 4  :
    {
      cudaFuncSetCacheConfig(convolveffdot10_f<FLAGS,noSteps,4>, cudaFuncCachePreferL1);
      convolveffdot10_f<FLAGS,noSteps,4><<<dimGrid, dimBlock, i1, cnvlStream>>>(cStack->d_kerData , cStack->d_iData, cStack->d_plainData, cStack->width, cStack->inpStride, offset);
      break;
    }
    case 5  :
    {
      cudaFuncSetCacheConfig(convolveffdot10_f<FLAGS,noSteps,5>, cudaFuncCachePreferL1);
      convolveffdot10_f<FLAGS,noSteps,5><<<dimGrid, dimBlock, i1, cnvlStream>>>(cStack->d_kerData , cStack->d_iData, cStack->d_plainData, cStack->width, cStack->inpStride, offset);
      break;
    }
    case 6  :
    {
      cudaFuncSetCacheConfig(convolveffdot10_f<FLAGS,noSteps,6>, cudaFuncCachePreferL1);
      convolveffdot10_f<FLAGS,noSteps,6><<<dimGrid, dimBlock, i1, cnvlStream>>>(cStack->d_kerData , cStack->d_iData, cStack->d_plainData, cStack->width, cStack->inpStride, offset);
      break;
    }
    case 7  :
    {
      cudaFuncSetCacheConfig(convolveffdot10_f<FLAGS,noSteps,7>, cudaFuncCachePreferL1);
      convolveffdot10_f<FLAGS,noSteps,7><<<dimGrid, dimBlock, i1, cnvlStream>>>(cStack->d_kerData , cStack->d_iData, cStack->d_plainData, cStack->width, cStack->inpStride, offset);
      break;
    }
    case 8  :
    {
      cudaFuncSetCacheConfig(convolveffdot10_f<FLAGS,noSteps,8>, cudaFuncCachePreferL1);
      convolveffdot10_f<FLAGS,noSteps,8><<<dimGrid, dimBlock, i1, cnvlStream>>>(cStack->d_kerData , cStack->d_iData, cStack->d_plainData, cStack->width, cStack->inpStride, offset);
      break;
    }
    case 9  :
    {
      cudaFuncSetCacheConfig(convolveffdot10_f<FLAGS,noSteps,9>, cudaFuncCachePreferL1);
      convolveffdot10_f<FLAGS,noSteps,9><<<dimGrid, dimBlock, i1, cnvlStream>>>(cStack->d_kerData , cStack->d_iData, cStack->d_plainData, cStack->width, cStack->inpStride, offset);
      break;
    }
    default :
    {
...
__host__  void convolveffdot10_f(cudaStream_t cnvlStream, cuFFdotBatch* batch, uint stack)
{
  ...
  dimGrid.y = ceil( cStack->harmInf->height / (float) ( CNV_DIMY ) );

  if      ( batch->flag & FLAG_STP_ROW )
    convolveffdot10_s<FLAG_STP_ROW>(dimGrid, dimBlock, 0, cnvlStream, batch, stack);
  else if ( batch->flag & FLAG_STP_PLN )
    convolveffdot10_s<FLAG_STP_PLN>(dimGrid, dimBlock, 0, cnvlStream, batch, stack);
  else
  {
