...
__global__ void mult50_k(const __restrict__ fcomplexcu* kernels, const __restrict__ fcomplexcu* datas, __restrict__ fcomplexcu* ffdot, int noPlains)
{
...
template<int FLAGS>
__host__  void mult50_s(dim3 dimGrid, dim3 dimBlock, int i1, cudaStream_t multStream, cuFFdotBatch* batch)
{
  switch (batch->noSteps)
  {
    case 1:
    {
      mult50_k<FLAGS,1><<<dimGrid, dimBlock, i1, multStream>>>(batch->d_kerData , batch->d_iData, batch->d_plainData, batch->noHarms);
      break;
    }
    case 2:
    {
      mult50_k<FLAGS,2><<<dimGrid, dimBlock, i1, multStream>>>(batch->d_kerData , batch->d_iData, batch->d_plainData, batch->noHarms);
      break;
    }
    case 3:
    {
      mult50_k<FLAGS,3><<<dimGrid, dimBlock, i1, multStream>>>(batch->d_kerData , batch->d_iData, batch->d_plainData, batch->noHarms);
      break;
    }
    case 4:
    {
      mult50_k<FLAGS,4><<<dimGrid, dimBlock, i1, multStream>>>(batch->d_kerData , batch->d_iData, batch->d_plainData, batch->noHarms);
      break;
    }
    case 5:
    {
      mult50_k<FLAGS,5><<<dimGrid, dimBlock, i1, multStream>>>(batch->d_kerData , batch->d_iData, batch->d_plainData, batch->noHarms);
      break;
    }
    case 6:
    {
      mult50_k<FLAGS,6><<<dimGrid, dimBlock, i1, multStream>>>(batch->d_kerData , batch->d_iData, batch->d_plainData, batch->noHarms);
      break;
    }
    case 7:
    {
      mult50_k<FLAGS,7><<<dimGrid, dimBlock, i1, multStream>>>(batch->d_kerData , batch->d_iData, batch->d_plainData, batch->noHarms);
      break;
    }
    case 8:
    {
      mult50_k<FLAGS,8><<<dimGrid, dimBlock, i1, multStream>>>(batch->d_kerData , batch->d_iData, batch->d_plainData, batch->noHarms);
      break;
    }
    default:
    {
...
__host__  void mult50_f(cudaStream_t multStream, cuFFdotBatch* batch)
{
  ...
  if      ( batch->flag & FLAG_ITLV_ROW )
    mult50_s<FLAG_ITLV_ROW>(dimGrid, dimBlock, 0, multStream, batch);
  else if ( batch->flag & FLAG_ITLV_PLN )
    mult50_s<FLAG_ITLV_PLN>(dimGrid, dimBlock, 0, multStream, batch);
  else
  {
