...
__global__ void mult23_k(const __restrict__ float2* kernels, const __restrict__ float2* inpData, __restrict__ float2* ffdot, const int width, const int stride, const int firstPlane )
{
  ...
  if ( tid < width )  // Valid thread  .
  {
    ...
    for ( int y = y0; y < y1; y+=cv_chunkSZ )
    {
      ...
      register float2 k0   = kernels[(c0+0 )*stride];
      register float2 k1   = kernels[(c0+1 )*stride];
      register float2 k2   = kernels[(c0+2 )*stride];
      register float2 k3   = kernels[(c0+3 )*stride];
      register float2 k4   = kernels[(c0+4 )*stride];
      register float2 k5   = kernels[(c0+5 )*stride];
      register float2 k6   = kernels[(c0+6 )*stride];
      register float2 k7   = kernels[(c0+7 )*stride];
      register float2 k8   = kernels[(c0+8 )*stride];
      register float2 k9   = kernels[(c0+9 )*stride];
      register float2 k10  = kernels[(c0+10)*stride];
      register float2 k11  = kernels[(c0+11)*stride];
...
__host__  void mult23_c(dim3 dimGrid, dim3 dimBlock, int i1, cudaStream_t multStream, cuFFdotBatch* batch, cuFfdotStack* cStack)
{
  int offset            = cStack->startIdx;
   ...
   switch (cStack->mulChunk)
   {
    ...
    case 1 :
    {
      mult23_k<FLAGS,noSteps,noPlns,1><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 2 :
    {
      mult23_k<FLAGS,noSteps,noPlns,2><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 3 :
    {
      mult23_k<FLAGS,noSteps,noPlns,3><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 4 :
    {
      mult23_k<FLAGS,noSteps,noPlns,4><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 5 :
    {
      mult23_k<FLAGS,noSteps,noPlns,5><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 6 :
    {
      mult23_k<FLAGS,noSteps,noPlns,6><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 7 :
    {
      mult23_k<FLAGS,noSteps,noPlns,7><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 8 :
    {
      mult23_k<FLAGS,noSteps,noPlns,8><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 9 :
    {
      mult23_k<FLAGS,noSteps,noPlns,9><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 10 :
    {
      mult23_k<FLAGS,noSteps,noPlns,10><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 11 :
    {
      mult23_k<FLAGS,noSteps,noPlns,11><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
    ...
    case 12 :
    {
      mult23_k<FLAGS,noSteps,noPlns,12><<<dimGrid, dimBlock, i1, multStream>>>((float2*)cStack->kernels->d_kerData, (float2*)cStack->d_iData, (float2*)cStack->d_planeMult, cStack->width, cStack->strideCmplx, offset);
      break;
    }
#endif
